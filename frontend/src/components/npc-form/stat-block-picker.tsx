...
// Monster search + the selected stat-block card, bound to the form's
// `statBlock` field. Selecting a monster also seeds the HP tracker (a
// cross-field effect) unless HP is already set — hence a `withForm` panel.
export const StatBlockPicker = withForm({
  ...npcFormOptions,
  render: function StatBlockPickerRender({ form }) {
    ...
    const statBlockMutation = useMutation({
      ...
      onSuccess: (block) => {
        form.setFieldValue('statBlock', block)
        // Seed the HP tracker from the monster unless already tracked.
        form.setFieldValue('maxHp', (prev) => prev || String(block.hitPoints))
        form.setFieldValue('currentHp', (prev) => prev || String(block.hitPoints))
        setMonsterSearch('')
        setSubmittedSearch('')
      },
    })
    ...
    return (
      <div className="space-y-2">
        ...
        <form.Subscribe selector={(state) => state.values.statBlock}>
          {(statBlock) =>
            statBlock ? (
              <div className="space-y-1 rounded-md border p-3 text-sm">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    onClick={() => form.setFieldValue('statBlock', null)}
                  >
                    Remove
                  </Button>
                </div>
